...
- Uses GPU device for initialization.
- Exports only wall positions as points for geometry verification.
"""
...
def main():
    ...
    # Create snapshot with only wall positions
    snap = hoomd.Snapshot()
    snap.configuration.box = [CFG.Lx, CFG.Ly, CFG.Lz, 0, 0, 0]
    snap.particles.N = len(walls)
    snap.particles.position[:] = walls  # Only positions, no types or diameters
    
    # Initialize simulation with snapshot
    sim.create_state_from_snapshot(snap)
    
    # Add GSD writer to export initial state
    gsd_writer = hoomd.write.GSD(
        filename='sim/out_loop/geometry.gsd',
        trigger=hoomd.trigger.Periodic(1),  # Write once at start
        mode='wb',
        filter=hoomd.filter.All()
    )
    sim.operations.writers.append(gsd_writer)
    
    # Write initial state (no steps run)
    sim.run(0)
    
    print("\n✅ DONE!")
    print("📁 sim/out_loop/geometry.gsd")
    print("Visualize the geometry in OVITO. Increase particle radius in OVITO if points are not visible.")
